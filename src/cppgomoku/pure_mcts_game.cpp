...
bool define_player(Player *&p, string& type, int color, double time_budget = 1.0) {
    ...
    if (type == "p1") {
        ...
        p = new PureMCTSPlayer(color, "Pure MCTS player", 10.0, 80000, time_budget); 
    } else {
...
int main(int argc, char *argv[]) {
    ...
    if (mode == "1") {
        ...
        if (!define_player(player1, player1_type, Board::kPlayerBlack, time_budget)) return 0;
        if (!define_player(player2, player2_type, Board::kPlayerWhite, time_budget)) return 0;
